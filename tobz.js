...
➸ *Wilayah:* +${region_name}`
        ...
        case '#help':
            ...
            break
        case '#elainagroup':
            tobz.reply(from, `Link Group RubyBot : https://chat.whatsapp.com\nJangan Lupa Join Ya Kak ${pushname}`, id)
            break
        case '#groupmenu':
        ...
        case '#info':
            ...
            break
        case '#bahasa':
            ruby.sendText(from, bahasalist)
            break
        case '#snk':
            ruby.reply(from, snk, id)
            break
        default:
            if (!isGroupMsg) return ruby.reply(from, 'Jika Ingin Menggunakan Bot Harap Masuk Ke Dalam Grup rubybot, Link Ada Di Bio atau Bisa Mengetik #rubybotgroup!\nJika Ingin Sewa Bot atau Bikin Bot Harap Ketik *#iklan*', id)
            if (command.startsWith('#')) {
                tobz.reply(from, `Maaf ${pushname}, Command *${args[0]}* Tidak Terdaftar Di Dalam *#menu*!`, id)
            }
            await ruby.sendSeen(from) 
            }
        }
    } catch (err) {
